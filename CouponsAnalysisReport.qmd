...
df$y <- factor(df$y,
                levels = t_01,
                labels = c("Rejected", "Accepted")) # Target

df$has_children <- factor(df$has_children,
                            levels = t_01,
                            labels = c("No children", "Has children"))

df$to_coupon_geq5min  <- factor(df$to_coupon_geq5min,
                                  levels = t_01,
                                  labels = t_NY)
df$to_coupon_geq15min <- factor(df$to_coupon_geq15min,
                                  levels = t_01,
                                  labels = t_NY)
df$to_coupon_geq25min <- factor(df$to_coupon_geq25min,
                                  levels = t_01,
                                  labels = t_NY)
