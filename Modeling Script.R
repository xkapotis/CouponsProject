...
head(feature_scores, 10)

#-----------------------------
# Step 2: Define staged datasets
#-----------------------------
stages <- list(
  stage1 = feature_scores$feature[1:3],
  stage2 = feature_scores$feature[1:6],
  stage3 = feature_scores$feature[1:12],
  stage4 = feature_scores$feature[1:24]  # all features
)

#-----------------------------
# Step 3: Function to train & evaluate models
#-----------------------------
evaluate_models <- function(features, stage_name) {
  cat("\n============================\n")
  cat(paste("Stage:", stage_name, "\n"))
  cat("============================\n")
  
  # Subset data
  train_sub <- train_data[, c(features, "y_factor")]
  test_sub  <- test_data[, c(features, "y_factor")]
  
  # 1️⃣ Logistic Regression
  model_log <- glm(y_factor ~ ., data = train_sub, family = binomial)
  prob_log  <- predict(model_log, test_sub, type = "response")
  pred_log  <- factor(ifelse(prob_log > 0.5, 1, 0), levels = c(0,1))
  cm_log <- confusionMatrix(pred_log, test_sub$y_factor)
  cat("\nLogistic Regression Metrics:\n")
  print(cm_log$overall[c("Accuracy", "Kappa")])
  
  # 2️⃣ Decision Tree
  model_tree <- rpart(y_factor ~ ., data = train_sub, method = "class")
  pred_tree <- predict(model_tree, test_sub, type = "class")
  cm_tree <- confusionMatrix(pred_tree, test_sub$y_factor)
  cat("\nDecision Tree Metrics:\n")
  print(cm_tree$overall[c("Accuracy", "Kappa")])
  
  # 3️⃣ Random Forest
  model_rf <- randomForest(y_factor ~ ., data = train_sub, ntree=200)
  pred_rf <- predict(model_rf, test_sub)
  cm_rf <- confusionMatrix(pred_rf, test_sub$y_factor)
  cat("\nRandom Forest Metrics:\n")
  print(cm_rf$overall[c("Accuracy", "Kappa")])
}

#-----------------------------
# Step 4: Run staged evaluations
#-----------------------------
for(stage_name in names(stages)) {
  evaluate_models(stages[[stage_name]], stage_name)
}
